          ...
          <div class="next-prev flex gap-2 my-4 items-end">
            <?php if($next = $page->nextListed()): ?>
              <a href="<?= $next->url() ?>" class="hover:rotate-45 transition-transform">
                <svg width="42" height="42" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M127.5 64C127.5 99.0701 99.0701 127.5 64 127.5C28.9299 127.5 0.5 99.0701 0.5 64C0.5 28.9299 28.9299 0.5 64 0.5C99.0701 0.5 127.5 28.9299 127.5 64Z" fill="white" stroke="black" class="dark:fill-slate-800 dark:stroke-white"/>
            ...
            <?php if($prev = $page->prevListed()): ?>
              <a href="<?= $prev->url() ?>" class="hover:rotate-45 transition-transform">
                <svg width="42" height="42" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path d="M0.499994 64C0.499991 28.9299 28.9299 0.500009 64 0.500006C99.0701 0.500003 127.5 28.9299 127.5 64C127.5 99.0701 99.0701 127.5 64 127.5C28.9299 127.5 0.499997 99.0701 0.499994 64Z" fill="white" stroke="black" class="dark:fill-slate-800 dark:stroke-white"/>
            ...
            <?php if($page->extra()->isNotEmpty()): ?>
              <div class="border border-black dark:border-white rounded-xl p-4 mt-4">
    ...
    <h1 class="uppercase text-page-title font-medium leading-none"><?= $page->title() ?></h1>
      ...
      <ul class="flex gap-2 mt-6">
        <?php foreach ($page->tags()->split() as $category): ?>
          <li class="text-base uppercase leading-none bg-purple dark:bg-slate-700 rounded-full py-1 px-4"><?= $category ?></li>
        <?php endforeach ?>
      </ul>
    </header>
      ...
      <div class="container max-w-screen-lg mt-8">
                ...
                <a href="<?= $block->link() ?>" class="group flex items-center gap-4 w-fit rounded-full py-1 pl-4 pr-1 border my-2 border-black dark:border-white hover:bg-black hover:text-white dark:hover:bg-white dark:hover:text-black transition-colors duration-300">
                    ...
                    <span class="transition-transform duration-300 group-hover:rotate-45">
                      <svg width="32" height="32" viewBox="0 0 128 128" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M127.5 64C127.5 99.0701 99.0701 127.5 64 127.5C28.9299 127.5 0.5 99.0701 0.5 64C0.5 28.9299 28.9299 0.5 64 0.5C99.0701 0.5 127.5 28.9299 127.5 64Z" fill="white" stroke="black" class="transition-colors duration-300 group-hover:fill-black group-hover:stroke-white dark:fill-slate-800 dark:stroke-white dark:group-hover:fill-white dark:group-hover:stroke-black"/>
        ...
        <h2 class="text-paragraph-title font-medium">Contributions</h2>
          <?php foreach($page->contributions()->toPages() as $contribution): ?>
            <a class="hover:bg-secondary dark:hover:bg-slate-700 bg-neutral-100 dark:bg-slate-800 rounded-full transition-colors duration-300 w-full" href="<?= $contribution->url() ?>">
              <article class="p-2 px-4 rounded-full overflow-x-scroll contribution-pill">
                ...
                <div class="flex flex-row items-center gap-2 md:gap-4 justify-between w-full whitespace-nowrap">
                  ...
                  <?php if($user = $contribution->author()->toUser()): ?>
                    <p class="text-sm rounded-full border border-black dark:border-white py-1 px-2 md:px-4 w-fit"><?= $user->username() ?></p>
                  <?php endif ?>

                  <p class="text-sm uppercase bg-neutral-200 dark:bg-slate-700 rounded-full py-1 px-4 h-fit ml-auto"><?= $contribution->tags() ?></p>

                  <?php $labos = $contribution->labos()->toPages(); if($labos->isNotEmpty()): ?>
                    <?php foreach($labos as $labo): ?>
                        <p class="text-sm md:text-sub text-black dark:text-white uppercase font-base inline-block"><?= $labo->title() ?></p>
                    <?php endforeach ?>
                  <?php endif ?>
                </div>
