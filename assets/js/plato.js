...
class FilterTag {
  ...
  inject () {
    ...
    // Listeners
    let filterText = this.type == 'labo' ? "Laboratoire : " : (this.type == 'category' ? "Catégorie : " : "Auteur·e : ")
  ...
  updateActive () {
    ...
    let isCategory = search.category && this.type == 'category' && this.id == search.category
    let isAuthor = search.author && this.type == 'author' && this.id == search.author
...
let updateSearchStatus = () => {
  ...
  document.querySelector('.filter[data-filter=labo]').classList.toggle('active', !!search.labo)
  document.querySelector('.filter[data-filter=category]').classList.toggle('active', !!search.category)
  document.querySelector('.filter[data-filter=author]').classList.toggle('active', !!search.author)
}
...
let initMobileSearchToggle = () => {
  document.getElementById('expand-search').removeEventListener('click', toggleMobileSearch)
  document.getElementById('expand-search').addEventListener('click', toggleMobileSearch)
}
...
let updateSearch = (type, id) => {
  ...
  search.labo = type == 'labo' ? (id == search.labo ? undefined : id) : undefined
  search.category = type == 'category' ? (id == search.category ? undefined : id) : undefined
  search.author = type == 'author' ? (id == search.author ? undefined : id) : undefined
  ...
  resetFrontLayer()
  if (isSearchActive()) {
    ctx2.fillStyle = colorMode == 'light' ? 'rgba(255, 255, 255, .5)' : 'rgba(0, 0, 12, .75)'
...
let initNodes = () => {
  ...
  if (mode == "desktop") {
    ...
    let x0 = w/2, y0 = h/2, data0 = data[0]
    ...
    console.log(nodes.length + ' nodes generated within ' + i + ' iterations in initNodes().')
