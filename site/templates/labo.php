    ...
    <aside class="sticky h-fit top-4 z-50">
          <div class="next-prev flex gap-2 my-4 items-end">
            ...
            <?php if($page->extra()->isNotEmpty()): ?>
              <div class="border border-black rounded-xl p-4 mt-4">
      ...
      <div class="container max-w-screen-lg mt-8">
        ...
        <ul class="flex gap-2 flex-wrap">
            <?php foreach($page->participants()->toUsers() as $user): ?>
                <li class="rounded-full border border-black py-1 px-4 w-fit"><?= $user->name() ?></li>
            <?php endforeach ?>
        </ul>
        ...
        <h2 class="text-paragraph-title font-medium">Contributions</h2>
          <?php foreach($page->contributions()->toPages() as $contribution): ?>
            <a class="hover:bg-secondary bg-neutral-100 rounded-full transition-colors duration-300 w-full" href="<?= $contribution->url() ?>">
